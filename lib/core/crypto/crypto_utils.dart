...
class CryptoUtils extends ICryptoUtils {
  ...
  Future<String> decrypt(String symKey, String encoded) async {
    ...
    final encodedData = deserialize(encoded);
    final b = dc.SecretBox.fromConcatenation(
  ...
  String serialize(
    ...
    if (type == EncodeOptions.TYPE_1) {
      if (senderPublicKey == null) throw WalletConnectError(code: -1, message: 'Missing sender public key for type 1 envelope');
  ...
  @override
  EncodingValidation validateDecoding(String encoded, {String? receiverPublicKey}) {
    final EncodingParams deserialized = deserialize(encoded);
    final String? senderPublicKey = deserialized.senderPublicKey != null ? hex.encode(deserialized.senderPublicKey!) : null;
  ...
  EncodingValidation validateEncoding({
    ...
    if (t == EncodeOptions.TYPE_1) {
      if (senderPublicKey == null) throw new WalletConnectError(code: -1, message: "Missing sender public key");
      if (receiverPublicKey == null) throw new WalletConnectError(code: -1, message: "Missing receiver public key");
    }
