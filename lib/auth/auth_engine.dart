...
import 'package:wallet_connect/models/auth/json_rpc_models.dart';
import 'package:wallet_connect/models/basic_models.dart';
import 'package:wallet_connect/models/jsonrpc/json_rpc_error.dart';
import 'package:wallet_connect/utils/address_utils.dart';
import 'package:wallet_connect/utils/auth_api_validators.dart';
import 'package:wallet_connect/utils/auth_signature.dart';
...
class AuthEngine implements IAuthEngine {
  ...
  Future<AuthRequestResponse> requestAuth({
    ...
    if (pTopic == null) {
      final newTopicAndUri = await core.pairing.create(methods: methods);
    ...
    WcAuthRequestRequest request = WcAuthRequestRequest(
      payloadParams: AuthPayloadParams.fromRequestParams(params),
    ...
    final int expiry = params.expiry ?? FIVE_MINUTES;
    ...
    await pairingTopics.set(responseTopic, pTopic);
  ...
  Future<void> _requestAuthResponseHandler({
    ...
    if (!isValid) {
      final resp = AuthResponse(
        id: id,
        topic: responseTopic,
        error: WalletConnectError(code: -1, message: 'Invalid signature')
      );
      onAuthResponse.broadcast(resp);
      completer.complete(resp);
    } else {
  ...
  Future<void> respondAuthRequest({
    ...
    final String responseTopic = core.crypto.getUtils().hashKey(receiverPublicKey);
  ...
  @override
  Map<int, StoredCacao> getCompletedRequestsForPairing({required String pairingTopic}) {
  ...
  String formatAuthMessage({
    ...
    final header = '${cacaoPayload.domain} wants you to sign in with your Ethereum account';
    ...
    final resources = cacaoPayload.resources != null && cacaoPayload.resources!.isNotEmpty
  ...
  void _onAuthRequest(String topic, JsonRpcRequest payload) async {
    try {
      ...
      final cacaoPayload = CacaoRequestPayload.fromPayloadParams(request.payloadParams);
