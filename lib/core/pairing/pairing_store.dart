...
class PairingStore implements IPairingStore {
  ...
  PairingInfo? get(String topic) {
    _checkInitialized();
    if (pairings.containsKey(topic)) return pairings[topic]!;
  ...
  Future<void> update(
    ...
    if (expiry != null) info.expiry = expiry;
    if (active != null) info.active = active;
    if (metadata != null) info.peerMetadata = metadata;
